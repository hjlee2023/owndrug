...
# ============================================
# 🧭 FDA 승인 미로 탈출 게임 (PC + 모바일 지원)
# ============================================

import streamlit as st
import streamlit.components.v1 as components

def maze_escape_game():
    """FDA 승인 미로 탈출 게임 - 약물을 FDA 승인까지 안내하기"""
    
    st.markdown("---")
    st.header("🧭 FDA 승인 미로 탈출")
    st.markdown("**신약을 FDA 승인까지 안내하세요!** | PC: 방향키 | 모바일: 화면 터치 또는 버튼")
    ...
    <body>
        <div id="gameContainer">
            ...
            <div id="message">💊 신약을 🏆 FDA 승인까지 안내하세요!</div>
            ...
            <div>
                <button class="button" onclick="startNewGame()">🎮 새 게임</button>
                <button class="button" onclick="nextLevel()">⏭️ 다음 레벨</button>
                <button class="button" onclick="resetLevel()">🔄 재시작</button>
            </div>
            
            <div id="gameOverModal">
                <h2 id="gameOverTitle">🎉 클리어!</h2>
                <p id="gameOverText"></p>
                <button class="button" onclick="nextLevel()">다음 레벨 ➡️</button>
                <button class="button" onclick="startNewGame()">처음부터 🔄</button>
            </div>
        </div>
        ...
        <script>
                ...
                function carve(x, y) {
                    ...
                    for (let direction of directions) {
                        ...
                        if (nx >= 0 && nx < cols && ny >= 0 && ny < rows && !maze[ny][nx].visited) {
                            maze[y][x][direction.dir] = false;
                            maze[ny][nx][direction.opposite] = false;
                            carve(nx, ny);
                        }
                    }
                }
            ...
            function winLevel() {
                ...
                document.getElementById('gameOverTitle').textContent = '🎉 FDA 승인 완료!';
                document.getElementById('gameOverText').innerHTML = `
                    <strong>레벨 ${gameState.level} 클리어!</strong><br>
                    이동 횟수: ${gameState.moves}<br>
                    시간: ${minutes}:${seconds.toString().padStart(2, '0')}
                `;
                document.getElementById('gameOverModal').style.display = 'block';
                document.getElementById('message').textContent = '🎊 축하합니다! FDA 승인을 받았습니다!';
            }
            ...
            function initLevel() {
                ...
                if (gameState.level <= 3) {
                    ...
                    gameState.cols = 12;
                    gameState.rows = 12;
                    gameState.cellSize = 41.6;
                } else {
                    gameState.cols = 15;
                    gameState.rows = 15;
                    gameState.cellSize = 33.3;
                }
                
                gameState.maze = generateMaze();
                gameState.player = { x: 0, y: 0 };
                gameState.goal = { x: gameState.cols - 1, y: gameState.rows - 1 };
                gameState.moves = 0;
                ...
                document.getElementById('message').textContent = '💊 신약을 🏆 FDA 승인까지 안내하세요!';
    ...
    with st.expander("🎯 게임 가이드"):
        ...
        **목표**: 💊 신약을 미로를 통과해 🏆 FDA 승인까지 안내하세요!
        ...
        - 항상 시작점에서 골까지 경로가 존재함
        
        ### 버튼 기능
        - **🎮 새 게임**: 레벨 1부터 다시 시작
        - **⏭️ 다음 레벨**: 현재 레벨 완료 후 다음 단계로
        - **🔄 재시작**: 현재 레벨을 새로운 미로로 재시작
        
        ### 팁
        - 막다른 길을 만나면 뒤로 돌아가세요
        - 오른쪽과 아래쪽으로 가는 게 일반적으로 가까움
        - 레벨이 높아질수록 미로가 복잡해집니다!
        
        **신약 개발처럼 길을 찾아 FDA 승인을 받으세요!** 💊🏆
        """)
